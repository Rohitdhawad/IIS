...
def list_entities(
    ...
    return db.scalars(
        statement.order_by(Entity.type, Entity.label).limit(limit)
    )
...
@router.get(
    "/{entity_id}/evidence",
    response_model=EntityEvidenceOut,
)
def entity_evidence(
    entity_id: str,
    db: Session = Depends(get_db),
    ...
    rows = list(
        db.scalars(
            ...
            .order_by(Relationship.observed_at)
        )
    )

    return EntityEvidenceOut(
        entity_id=entity_id,
        outgoing_relationships=[
            RelationshipOut.model_validate(row)
            for row in rows
...
def get_case_graph(
    ...
    return graph_query_service.get_case_graph(
        case_id=case_id,
        entity_id=entity_id,
        depth=depth,
